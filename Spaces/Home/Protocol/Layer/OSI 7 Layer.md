...
PDU
---
정보를 추가한 계층에 따라 PDU를 지칭하는 이름과 포함하는 내용이 달라진다.[[1]](https://ko.wikipedia.org/wiki/%ED%94%84%EB%A1%9C%ED%86%A0%EC%BD%9C_%EB%8D%B0%EC%9D%B4%ED%84%B0_%EB%8B%A8%EC%9C%84#cite_note-1)

- 전송계층 : [세그먼트](https://ko.wikipedia.org/wiki/%EC%84%B8%EA%B7%B8%EB%A8%BC%ED%8A%B8 "세그먼트")(segment)

1. 발신지 [포트](https://ko.wikipedia.org/wiki/%ED%8F%AC%ED%8A%B8 "포트") : 발신하는 application의 포트
2. 목적지 포트 : 수신해야 할 application의 포트
3. 순서 번호 : 순차적 전송할 경우 순서를 붙이며, 순서가 어긋나면 목적지 프로토콜이 이를 바로 잡는다.
4. [오류검출코드](https://ko.wikipedia.org/w/index.php?title=%EC%98%A4%EB%A5%98%EA%B2%80%EC%B6%9C%EC%BD%94%EB%93%9C&action=edit&redlink=1 "오류검출코드 (없는 문서)") : 발신지와 목적지 프로토콜은 세그먼트를 연산하여 오류 검출 코드를 각각 만든다. 만약 발신지에서 전송한 세그먼트에 포함된 오류 검출 코드와 목적지에서 만든 오류 검출 코드가 다르다면 전송되는 과정에서 오류가 발생한 것이다. 이 경우, 수신측은 그 세그먼트를 폐기하고 복구 절차를 밟는다. 오류검출코드는 [체크섬](https://ko.wikipedia.org/wiki/%EC%B2%B4%ED%81%AC%EC%84%AC "체크섬"), [프레임 체크 시퀀스](https://ko.wikipedia.org/w/index.php?title=%ED%94%84%EB%A0%88%EC%9E%84_%EC%B2%B4%ED%81%AC_%EC%8B%9C%ED%80%80%EC%8A%A4&action=edit&redlink=1 "프레임 체크 시퀀스 (없는 문서)")라고도 부른다.

- 네트워크 계층 :[패킷](https://ko.wikipedia.org/wiki/%ED%8C%A8%ED%82%B7 "패킷")(packet)
