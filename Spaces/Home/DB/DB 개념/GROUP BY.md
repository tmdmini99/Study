...
|            |                                                       |
| ---------- | ----------------------------------------------------- |
| `COUNT(*)` | null 포함 모든 행 수를 셈                                     |
|            |                                                       |
| `COUNT(1)` | `1`은 null이 될 수 없으므로, 사실상 모든 행을 셈 (조건이 있으면 조건에 맞는 행 수) ||

- 테이블에서 조건에 맞는 **행의 수(row 수)**를 셉니다.
    
- 여기서 `1`은 상수이며, `SELECT`된 각 행마다 **항상 존재하는 값**이므로,
    
- **조건에 일치하는 행이 몇 개인지를 셀 때** `COUNT(*)`와 거의 동일하게 동작합니다.
