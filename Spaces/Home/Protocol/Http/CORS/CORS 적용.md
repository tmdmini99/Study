...
**RestTemplate**을 사용한 외부 API 호출

- `RestTemplate`은 Spring에서 **외부 HTTP 요청**을 쉽게 보낼 수 있게 해주는 클래스입니다.
- `restTemplate.getForEntity(url, byte[].class)`를 통해, 클라이언트가 전달한 `url`로 요청을 보내고, 그 응답을 `byte[]`로 받습니다.
...
- **단순성**: 클라이언트가 외부 API에 직접 접근하므로 구현이 간단합니다. 서버에서 추가적인 로직을 작성할 필요가 없습니다.
- **성능**: 요청이 서버를 거치지 않고 클라이언트와 외부 API 간에 직접 이루어지기 때문에 응답 시간이 짧아질 수 있습니다.

**단점:**

- **보안**: 외부 API에 대한 직접 접근은 보안 위험을 초래할 수 있습니다. 클라이언트가 API 키를 노출할 위험이 있습니다.
- **에러 처리**: 에러 발생 시 클라이언트에서 직접 처리해야 하며, 서버에서 보다 유용한 에러 메시지를 제공하기 어렵습니다.
- **CORS 설정 복잡성**: 다양한 출처를 허용해야 할 경우 CORS 설정이 복잡해질 수 있습니다.

### 2. **프록시 컨트롤러 사용**

**장점:**

- **보안**: 클라이언트가 외부 API의 세부 사항을 알 필요가 없으므로 API 키와 같은 민감한 정보가 노출되지 않습니다.
- **데이터 처리**: 서버에서 요청을 가공하거나 변환할 수 있어 더 유연한 데이터 처리 및 응답 구성이 가능합니다.
- **에러 관리**: 서버에서 에러를 관리할 수 있어 사용자에게 보다 나은 에러 메시지를 제공할 수 있습니다.
- **API 버전 관리**: 외부 API가 변경되더라도 서버에서만 수정하면 되므로 클라이언트에 미치는 영향을 최소화할 수 있습니다.


### 어떤 것을 선택해야 할까?

- **보안이 중요한 경우**: 외부 API에 민감한 정보가 포함되어 있다면 프록시 컨트롤러를 사용하는 것이 더 안전합니다.
- **데이터 가공이 필요한 경우**: 응답 데이터에 대한 추가 처리나 변환이 필요하다면 프록시 컨트롤러가 유리합니다.
- **에러 처리를 중앙 집중식으로 관리하고 싶다면**: 사용자에게 유용한 에러 메시지를 제공하고, 서버에서 모든 요청을 관리하고 싶다면 프록시 컨트롤러가 좋습니다.
- **단순한 요청 및 빠른 성능이 필요한 경우**: 복잡한 로직이 필요 없고, 클라이언트에서 직접 API를 호출해도 되는 경우에는 `@CrossOrigin`을 사용하는 것이 더 나을 수 있습니다.
