...
- `@>` 연산자는 `"오른쪽 배열"`의 **모든 요소가 왼쪽 배열에 포함**되어 있는지를 검사.
- `&&` 대신 `@>` 사용하면 `"17과 18이 모두 존재하는 행만 반환"` 됨.
...
- `contents` 컬럼의 값이 **우변 정규표현식과 매칭**되면 `true`가 됩니다.
    
- 즉, **정규식에 해당하는 문자열이 포함되어 있는지**를 확인하는 조건입니다.


```sql
contents ~ CONCAT('data="([^"]*,)?', #{item}, '(,[^"]*)?"')

```


`contents`에 다음 조건을 만족하는 문자열이 포함되어야 `true`:

- `data="`로 시작하고
    
- `#{item}`이 **콤마로 구분된 값들 중 하나**로 존재하며
    
- 앞이나 뒤에 다른 숫자가 **있을 수도 있음**


정규식 예시 (예: `item`이 `46`일 때)
...
즉, `data="..."` 안에 `46`이라는 값이 **중간이든 앞이든 뒤든 포함되어 있을 경우** 매칭됩니다.
