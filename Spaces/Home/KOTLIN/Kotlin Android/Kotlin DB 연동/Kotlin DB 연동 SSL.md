...
class SignUpActivity : AppCompatActivity() {  
    ...
    // TrustManager 설정: 모든 인증서를 신뢰하도록 설정합니다 (테스트 환경에서만 사용)  
        ...
        return withContext(Dispatchers.IO) {  
            ...
            try {  
                ...
                val connection = (url.openConnection() as HttpsURLConnection).apply {  
                    sslSocketFactory = getTrustAllHostsSSLSocketFactory() // TrustManager 적용  
                    hostnameVerifier = HostnameVerifier { _, _ -> true } // 호스트네임 검증 비활성화 (테스트 환경에서만)  
